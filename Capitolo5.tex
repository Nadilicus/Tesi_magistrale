...
\end{theorem}
\begin{proof}
    For easier computation suppose $S \subset \A^{2,1}$. Denote by $X^\top$ and $X^\perp$ the tangent and normal part of $X$ with respect to $TS$. Using Equation \ref{eq:levi} we have
        \[
        (\nabla_X \nabla_Y Z)^\top = \nabla^I_X \nabla^I_Y Z + \II(Y,Z) \nabla_X \nu \ ,
    \] 
    \[
    ...
    \]
    Computiting the tangential part of the Riemann tensor we have
    \[
    \begin{split}
    ...
    \end{split}
    \]
    where we used that the normal part of the Riemann tensor is null when the sectional curvature is constant.
\end{proof}
