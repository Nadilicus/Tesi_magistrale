...
\begin{proof}
    \red{mettere proof?}\\
    \red{semprerebbe che esistano coordinate per cui "$\nabla_{e_1} \nu = f e_2$ e $\nabla_{e_2} \nu = g e_1$"}
\end{proof}
...
\begin{proof}
    ...
    \]
    \red{da rivedere la fine, l'apparizione di $\I$ non mi convince}
\end{proof}
...
\begin{proposition} \label{prop:Gaussian curvature}
    ...
    \]
\end{proposition}
For a proof see \cite{Krasnov_2007}. If we apply the proposition to the tensors $id \pm \mathcal{J} \circ B$, equation \ref{eq:deteminant differential projections} shows that the pull-back metrics defined in Proposition \ref{prop:left right pull-back metric}, when non-degenerate. are always hyperbolic.