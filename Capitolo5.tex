...
\red{Espandere, mettere dim di eq di Gauss Codazzi}
...
\red{||||||}
Let us prove this, supposing $S\subset \A^{2,1}$ for easier computation. Let $R$ and $R^\I$ be the Riemann curvature tensor on $\A^{2,1}$ and $S$ respectively. We denote by $X^\top$ and $X^\perp$ the tangent and normal part of $X$ with respect to $S$. Using Equation \ref{eq:levi} we have
\[
    (\nabla_X \nabla_Y Z)^\top = \nabla^I_X \nabla^I_Y Z + \II(Y,Z) \nabla_X \nu \ ,
\] 
and
...
Now, using that the normal part of the Riemann tensor is null when the sectional curvature is constant, we have
\[
\begin{split}
    0 = \langle R(X,Y)Z,\nu \rangle & = 
    \partial_Y (\II (X,Z)) -\II(\nabla_Y X,Z) -\II(X, \nabla_Y Z) -\partial_X (\II (Y,Z)) \\ 
    & + \II(\nabla_X Y,Z)+\II(Y, \nabla_X Z) = (\nabla_Y \II)(X,Z) - (\nabla_X \II)(Y,Z).
\end{split}
\]
