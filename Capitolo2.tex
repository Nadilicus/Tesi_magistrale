\chapter{Anti-de Sitter Space}
\red{Aggiungere vaga introduzione}
...
By definition $\A^{n,1}$ is identified with the subset of $\mathbb{R} P^{n+1}$ given by the timelike direction of $\R^{n+2}$:
\[
\A^{n,1} = \{ \left[ x \right]\in \mathbb{R} P^{n+1} \ | \ q_{n,2}<0  \}.
\]
Like in hyperbolic geometry, the boundary of $\A^{n,2}$ is the projectivization of the set of lightlike vectors in $\R^{n,2}$:
\[
\partial\A^{n,1} = \{ \left[ x \right]\in \mathbb{R} P^{n+1} \ | \ q_{n,2}=0  \}.
\]
Isometries of $\A^{n,1}$ induce projective transformation which preserve $\partial \A^{n,1}$.\\

\red{Forse aggiungere la metrica al bordo}
...
\textit{In the Klein model}. In analogy with the hyperbolic case, geodesics in the Klein model are intersection of projective lines with the domain $A^{n,1} \subset \mathbb{R}P^{n+1}$. From the study of geodesics in the quadric model follows that
\begin{itemize}
...
\red{Forse osservazioni se aggiungo la metrica al bordo}
