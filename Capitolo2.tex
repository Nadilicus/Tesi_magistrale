...
\noindent \textit{In the universal cover}. In the universal cover $\AS^{n,1}$ geodesics are the lifts of the geodesics of $\H^{n,1}$ or $\A^{n,1}$. Every lightlike or spacelike geodesic in $\H^{n,1}$ and $\A^{n,1}$ is topologically a line, therefore it has a countable number of lifts to $\AS^{n,1}$. On the other hand in $\H^{n,1}$ and $\A^{n,1}$ timelike geodesics are topologically circles and are in bijection with timelike geodesics of $\AS^{n,1}$.\\
...
Because of Theorem \ref{ConformalMetric} we can replace the Poincaré metric by the conformally equivalent metric, and often easier to manage in calculation, given by:
