...
\end{equation}
We turn now our attention to the case where $M$ is a compact surface. As a complex manifold, $M$ admits complex coordinate $z=x + iy$ which defines on the complexified tangent space the vector fields
\[
...
is a compact subset of $M$. For an easier computation, suppose that $M$ is a submanifold of $N$ so that $f = id_M$. The variation of volume is then
\[
...
\begin{proof}
    ...
    \[
       \Pi_l^*g_{\H^2} + \Pi_r^*g_{\H^2} = 2I + 2I(\mathcal{J}B \cdot,\mathcal{J}B \cdot) = 2I + 2I(B \cdot,B \cdot)= 2(I+ \III) \ .
    \]
